...
| `31h` `_DPARM` | the volume's parameters as Nextor answers them: 512-byte sectors, the cluster size, the reserved sectors, the FATs, the root entries, the first root and data sectors counted from the volume's start, the clusters + 2 as the maximum cluster, the boot sector's media descriptor and volume id (`−1` when it has none), dirty flag 0; the total sectors are the data sector plus the whole clusters' — a partial cluster at the end is left out — and read 0, as does the sectors per FAT, when the field cannot hold them, as MSX-DOS 2 answers for a volume too large to describe; the same total in 32 bits at offset 24, and 0 for FAT12 or 1 for FAT16 at offset 28 |
