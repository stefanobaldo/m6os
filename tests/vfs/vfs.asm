...
; rest as process 0, through the syscalls a program uses, and starts a
; three-page reader from the volume for what only a process can show. The report is on
...
; --- step 10: a process reads the big file -----------------------------------
        ...
        ld      hl,t_k10
        k_call  API_CON_PUTS
        ld      hl,p_reader             ; /reader, a program on the volume
        ld      de,0
        ld      bc,t_fdinh
        xor     a
        sys     SYS_SPAWNV
...
root_names: db  "NEXTOR.SYS",0,"COMMAND2.COM",0,"VFS.COM",0,"AUTOEXEC.BAT",0
            db  "BIN",0,"DATA",0,"ETC",0,"SHORT",0,"READER",0,0
