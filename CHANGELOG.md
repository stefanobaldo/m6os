...
- MSX-DOS 2 programs on a 128K machine found no free segment, where
  MSX-DOS 2 leaves one: SofaRun's "Forget last flashed ROM" stopped with
  `*** Out of segments`, and a program that needs a segment of its own
  could not have one. The shell's segment, frozen while the program runs,
  is now counted free and is the program's when it asks for it, its bytes
  kept in `/m6.swp` on the boot volume for the run. See
